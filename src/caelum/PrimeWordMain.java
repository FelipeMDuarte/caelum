/*Programa para verificar se uma palavra é prima ou não
 * Felipe Magalhães Duarte - UFRJ */
package caelum;
...
public class PrimeWordMain {
	...
	public static void main(String[] args) throws FileNotFoundException {
		//Inicialização de variáveis e leitores de arquivos
		FileReader fileReader =  new FileReader("input.txt");
		Scanner scanner = new Scanner(fileReader);
		String read;	
		int valueRead =0 ;
		//Loop para ler o arquivo até EOF
		while(scanner.hasNextLine()){
			read = scanner.nextLine();
			//Chamada da função para transformar as letras da palavra em numeros e soma-los
			valueRead = valueTransform(read);
			//Chamada e verificação do retorno da função que define se um numero é primo ou não
	...
	/*Função para transformação de letras nos numeros pedidos e ao mesmo tempo somar esses numeros todos
	 * Aqui poderiamos usar um HashMap, porém achei que seria exagero */
